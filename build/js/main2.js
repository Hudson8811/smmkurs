...
const cardsContainer = document.querySelector(".custom-slider");
...
class CardCarousel extends DraggingEvent {
	constructor(container, controller = undefined) {
		...
		this.cards = container.querySelectorAll(".custom-slider-card")
	...
	updateCards(card, data) {
		...
		if (data.scale || data.scale == 0) {
			...
			if (Math.abs(data.x)>0) {
				card.style.opacity = data.scale*.5;
			}
			//console.log((Math.abs(data.x)));
			if (Math.abs(data.x)>1) {
